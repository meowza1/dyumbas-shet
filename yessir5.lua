...
local Library do
        ...
        Library = {
                ...
                Folders = {
                        Directory = "workspace/scriptname",
                        Configs = "workspace/scriptname/Configs",
                        Assets = "workspace/scriptname/Assets"
                },
                Images = {
...
do
        ...
        self.funcs = { 
                ...
                apply_desync = function()
                        if not Stranded.desync.enabled or not Stranded.desync.active then
                                if self.desync_cache and self.desync_cache.has_original then
                                        local character = self.vars.local_player and self.vars.local_player.Character
                                        if character then
                                                local hrp = character:FindFirstChild("HumanoidRootPart")
                                                if hrp then
                                                        hrp.CFrame = self.desync_cache.original_cframe
                                                        hrp.AssemblyLinearVelocity = self.desync_cache.original_velocity
                                                end
                                        end
                                        self.desync_cache.has_original = false
                                end
                                return
                        end
                        ...
                        if not self.desync_cache then
                                self.desync_cache = {
                                        has_original = false,
                                        last_mode = nil,
                                        server_only_applied = false
                                }
                        end
                        
                        local current_desync_mode = Stranded.desync.mode or "Random"
                        if self.desync_cache.last_mode ~= current_desync_mode then
                                self.desync_cache.has_original = false
                                self.desync_cache.last_mode = current_desync_mode
                        end
                        
                        if not self.desync_cache.has_original then
                                self.desync_cache.original_cframe = hrp.CFrame
                                self.desync_cache.original_velocity = hrp.AssemblyLinearVelocity
                                self.desync_cache.has_original = true
                        end
                        
                        local desync_mode = Stranded.desync.mode or "Random"
                        local random_offset = Vector3.new(0, 0, 0)
                        local base_pos = self.desync_cache.original_cframe.Position
                        local yaw_offset = 0
                        ...
                        elseif desync_mode == "Spin" then
                                ...
                                yaw_offset = math.rad((tick() * (Stranded.desync.yaw_spin_speed or 180)) % 360)
                        else
                                local x = math.random(Stranded.desync.min_x * 100, Stranded.desync.max_x * 100) / 100
                                local y = math.random(Stranded.desync.min_y * 100, Stranded.desync.max_y * 100) / 100
                                local z = math.random(Stranded.desync.min_z * 100, Stranded.desync.max_z * 100) / 100
                        ...
                        local _, current_yaw = self.desync_cache.original_cframe:ToEulerAnglesYXZ()
                        ...
                        local fake_velocity = random_vel_direction * random_vel_magnitude
                        
                        local server_only = Stranded.desync.server_only
                        
                        if server_only then
                                hrp.CFrame = fake_cframe
                                hrp.AssemblyLinearVelocity = fake_velocity
                                
                                local rs = self.vars.rs or game:GetService("RunService")
                                rs:BindToRenderStep("DesyncRestore", Enum.RenderPriority.Camera.Value - 2, function()
                ...
                update_server_hrp_visualizer = function()
                        ...
                        if Stranded.desync.enabled and Stranded.desync.active then
                                local desync_mode = Stranded.desync.mode or "Random"
                                local random_offset = Vector3.new(0, 0, 0)
                                local base_pos = server_cframe.Position
                                        ...
                                        if local_player and local_player.Character then
                                                ...
                                                if is_whitelisted then
                                                        ...
                                                        if current_time - last_fire_time >= (Stranded.triggerbot.settings.delay or 0.1) then
                                                                -- Check if player is strafing (moving left/right more than forward/backward)
                                                                local is_strafing = false
                                                                if local_player and local_player.Character then
                                                                        local humanoid = local_player.Character:FindFirstChildOfClass("Humanoid")
                                                                        ...
                                                                        if humanoid and hrp then
                                                                                ...
                                                                                if move_direction.Magnitude > 0.1 then
                                                                                        local forward_backward = math.abs(move_direction:Dot(hrp.CFrame.LookVector))
                                                                                        local left_right = math.abs(move_direction:Dot(hrp.CFrame.RightVector))
                                                                                        is_strafing = left_right > forward_backward
                                                                                end
                                                                        end
                                                                end
                                                                
                                                                -- Fire if either mouse is pressed OR player is strafing
                                                                if uis:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) or is_strafing then
                                                                        local camera = self.vars.camera or workspace.CurrentCamera
                                                                        if camera then
...
local success_ui, err = pcall(function()
        ...
        DesyncSection:Toggle({Name = "Server Only Desync", Flag = "Desync Server Only", Default = Stranded.desync.server_only, Callback = function(Value)
                Stranded.desync.server_only = Value
        end})

        local desync_min_x, desync_max_x, desync_min_y, desync_max_y, desync_min_z, desync_max_z
        local desync_void_depth, desync_sky_height, desync_horizontal_jitter, desync_jitter_strength
        local desync_circle_radius, desync_circle_speed, desync_spin_speed
        local desync_update_interval, desync_velocity_scale
        local desync_min_rotation, desync_max_rotation, desync_velocity
        
        local function update_desync_visibility(mode)
                local function set_visible(element, visible)
                        if element and element.SetVisibility then
                                element:SetVisibility(visible)
                        end
                end
                set_visible(desync_min_x, mode == "Random" or mode == "Spin")
                set_visible(desync_max_x, mode == "Random" or mode == "Spin")
                set_visible(desync_min_y, mode == "Random" or mode == "Spin")
                set_visible(desync_max_y, mode == "Random" or mode == "Spin")
                set_visible(desync_min_z, mode == "Random" or mode == "Spin")
                set_visible(desync_max_z, mode == "Random" or mode == "Spin")
                set_visible(desync_void_depth, mode == "Void")
                set_visible(desync_sky_height, mode == "Sky")
                set_visible(desync_horizontal_jitter, mode == "Void" or mode == "Sky")
                set_visible(desync_jitter_strength, mode == "Jitter")
                set_visible(desync_circle_radius, mode == "Circle")
                set_visible(desync_circle_speed, mode == "Circle")
                set_visible(desync_spin_speed, mode == "Spin")
                set_visible(desync_min_rotation, mode == "Random" or mode == "Spin")
                set_visible(desync_max_rotation, mode == "Random" or mode == "Spin")
                set_visible(desync_velocity, mode == "Random" or mode == "Spin")
                set_visible(desync_velocity_scale, true)
                set_visible(desync_update_interval, true)
        end

        local desync_mode_dropdown = DesyncSection:Dropdown({Name = "Mode", Flag = "Desync Mode", Default = Stranded.desync.mode, Items = {"Random", "Void", "Sky", "Jitter", "Circle", "Spin"}, Callback = function(Value)
                Stranded.desync.mode = tostring(Value) or "Random"
                update_desync_visibility(Stranded.desync.mode)
        end})
        
        desync_min_x = DesyncSection:Slider({Name = "Min X", Min = -100, Max = 100, Default = Stranded.desync.min_x, Decimals = 0, Compact = true, Flag = "Desync Min X", Callback = function(Value)
                Stranded.desync.min_x = Value
        end})
        
        desync_max_x = DesyncSection:Slider({Name = "Max X", Min = -100, Max = 100, Default = Stranded.desync.max_x, Decimals = 0, Compact = true, Flag = "Desync Max X", Callback = function(Value)
                Stranded.desync.max_x = Value
        end})
        
        desync_min_y = DesyncSection:Slider({Name = "Min Y", Min = -100, Max = 100, Default = Stranded.desync.min_y, Decimals = 0, Compact = true, Flag = "Desync Min Y", Callback = function(Value)
                Stranded.desync.min_y = Value
        end})
        
        desync_max_y = DesyncSection:Slider({Name = "Max Y", Min = -100, Max = 100, Default = Stranded.desync.max_y, Decimals = 0, Compact = true, Flag = "Desync Max Y", Callback = function(Value)
                Stranded.desync.max_y = Value
        end})
        
        desync_min_z = DesyncSection:Slider({Name = "Min Z", Min = -100, Max = 100, Default = Stranded.desync.min_z, Decimals = 0, Compact = true, Flag = "Desync Min Z", Callback = function(Value)
                Stranded.desync.min_z = Value
        end})
        
        desync_max_z = DesyncSection:Slider({Name = "Max Z", Min = -100, Max = 100, Default = Stranded.desync.max_z, Decimals = 0, Compact = true, Flag = "Desync Max Z", Callback = function(Value)
                Stranded.desync.max_z = Value
        end})

        desync_void_depth = DesyncSection:Slider({Name = "Void Depth", Min = 25, Max = 1000, Default = Stranded.desync.void_depth, Decimals = 0, Compact = true, Flag = "Desync Void Depth", Callback = function(Value)
                Stranded.desync.void_depth = Value
        end})

        desync_sky_height = DesyncSection:Slider({Name = "Sky Height", Min = 25, Max = 1000, Default = Stranded.desync.sky_height, Decimals = 0, Compact = true, Flag = "Desync Sky Height", Callback = function(Value)
                Stranded.desync.sky_height = Value
        end})

        desync_horizontal_jitter = DesyncSection:Slider({Name = "Horizontal Jitter", Min = 0, Max = 100, Default = Stranded.desync.horizontal_jitter, Decimals = 0, Compact = true, Flag = "Desync Horizontal Jitter", Callback = function(Value)
                Stranded.desync.horizontal_jitter = Value
        end})

        desync_jitter_strength = DesyncSection:Slider({Name = "Jitter Strength", Min = 1, Max = 100, Default = Stranded.desync.jitter_strength, Decimals = 0, Compact = true, Flag = "Desync Jitter Strength", Callback = function(Value)
                Stranded.desync.jitter_strength = Value
        end})

        desync_circle_radius = DesyncSection:Slider({Name = "Circle Radius", Min = 1, Max = 100, Default = Stranded.desync.circle_radius, Decimals = 0, Compact = true, Flag = "Desync Circle Radius", Callback = function(Value)
                Stranded.desync.circle_radius = Value
        end})

        desync_circle_speed = DesyncSection:Slider({Name = "Circle Speed", Min = 1, Max = 50, Default = Stranded.desync.circle_speed, Decimals = 0, Compact = true, Flag = "Desync Circle Speed", Callback = function(Value)
                Stranded.desync.circle_speed = Value
        end})

        desync_spin_speed = DesyncSection:Slider({Name = "Spin Speed", Min = 1, Max = 1000, Default = Stranded.desync.yaw_spin_speed, Decimals = 0, Compact = true, Flag = "Desync Spin Speed", Callback = function(Value)
                Stranded.desync.yaw_spin_speed = Value
        end})

        desync_update_interval = DesyncSection:Slider({Name = "Update Interval", Min = 0, Max = 1, Default = Stranded.desync.desync_interval, Decimals = 2, Compact = true, Flag = "Desync Update Interval", Callback = function(Value)
                Stranded.desync.desync_interval = Value
        end})

        desync_velocity_scale = DesyncSection:Slider({Name = "Velocity Scale", Min = 0, Max = 5, Default = Stranded.desync.velocity_scale, Decimals = 2, Compact = true, Flag = "Desync Velocity Scale", Callback = function(Value)
                Stranded.desync.velocity_scale = Value
        end})
        
        desync_min_rotation = DesyncSection:Slider({Name = "Min Rotation", Min = 0, Max = 360, Default = Stranded.desync.min_rotation, Decimals = 0, Compact = true, Flag = "Desync Min Rotation", Callback = function(Value)
                Stranded.desync.min_rotation = Value
        end})
        
        desync_max_rotation = DesyncSection:Slider({Name = "Max Rotation", Min = 0, Max = 360, Default = Stranded.desync.max_rotation, Decimals = 0, Compact = true, Flag = "Desync Max Rotation", Callback = function(Value)
                Stranded.desync.max_rotation = Value
        end})
        
        desync_velocity = DesyncSection:Slider({Name = "Velocity", Min = 0, Max = 999999, Default = (Stranded.desync.max_velocity == math.huge and 999999 or Stranded.desync.max_velocity), Decimals = 0, Compact = true, Flag = "Desync Velocity", Callback = function(Value)
