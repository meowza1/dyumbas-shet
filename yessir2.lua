-- table should be under this


--[[
  Project: stranded.lol
  ...
  Copyright (c) 2025 Codeine, Wack
  All rights reserved.

  Permission is granted to use, view, and modify this file for personal
  or educational purposes only. Redistribution or commercial use without
...
do
	...
	self.funcs = { 
		...
		target_strafe_update = function()
			if not Stranded.target_strafe.enabled then return end
			local lp = self.vars.local_player
			if not lp or not lp.Character then return end
			local hrp = lp.Character:FindFirstChild("HumanoidRootPart")
			if not hrp then return end

			local flag = Library and Library.Flags and Library.Flags["Target Strafe Hotkey"] or nil
			local should_strafe = false
			if Stranded.target_strafe.mobile_support and self.vars.uis.TouchEnabled and not self.vars.uis.KeyboardEnabled then
				should_strafe = true
			elseif flag then
				...
				if mode == "Always" then
					should_strafe = true
				elseif mode == "Toggle" then
					should_strafe = flag.Toggled or false
				else
					local key = flag.Key
					if type(key) == "string" then key = Enum.KeyCode[key] end
					if not key then key = Enum.KeyCode[Stranded.target_strafe.keybind or "E"] or Enum.KeyCode.E end
					should_strafe = key and self.vars.uis:IsKeyDown(key) or false
				end
			end
			if not should_strafe then return end

			self.target_strafe_cache = self.target_strafe_cache or {rotation_angle = 0, last_update = tick()}
			local cache = self.target_strafe_cache
			local target_part, target_player

			local selected = Stranded.target_strafe.selected_target
			if type(selected) == "string" and selected ~= "" and selected ~= "Auto" then
				local q = string.lower(selected)
				for _, plr in ipairs(self.vars.players:GetPlayers()) do
					if plr ~= lp then
						local user = string.lower(plr.Name or "")
						local disp = string.lower(plr.DisplayName or "")
						local label = string.lower((plr.Name or "") .. " (@" .. (plr.DisplayName or "") .. ")")
						if user == q or disp == q or label == q then
							target_player = plr
							if plr.Character then
								target_part = plr.Character:FindFirstChild("HumanoidRootPart") or plr.Character:FindFirstChild("Head")
							end
							break
						end
					end
			...
			if not target_part and Script and Script.Targeting and Script.Targeting.Target then
			...
			cache.sticky_target_part = target_part
			cache.sticky_target_player = target_player

			local now = tick()
			local dt = math.max(now - (cache.last_update or now), 0.001)
			cache.last_update = now
			local speed = Stranded.target_strafe.speed or 10
			local height = Stranded.target_strafe.height or 10
			local radius = Stranded.target_strafe.distance or 10
			local direction_mul = (Stranded.target_strafe.direction == "Counter Clockwise") and -1 or 1
			local pattern = Stranded.target_strafe.pattern or "Normal"
			local x,z,y = 0,0,height
			cache.rotation_angle = (cache.rotation_angle + (dt * speed * 0.5 * direction_mul)) % (math.pi*2)
			local a = cache.rotation_angle

			if pattern == "Trigonometric" then
				local t = now * speed * direction_mul
				x,z,y = math.cos(t)*radius, math.sin(t)*radius, math.sin(t)*height
			elseif pattern == "Elliptical" then
				x,z,y = math.cos(a)*radius, math.sin(a)*(radius*0.6), math.sin(a*1.5)*height*0.4 + height
			elseif pattern == "Figure-8" then
				x,z,y = math.sin(a)*radius, math.sin(a*2)*radius*0.5, math.cos(a)*height*0.3 + height
			elseif pattern == "Jitter" then
				local j = Stranded.target_strafe.jitter_amount or 0
				x,z,y = math.cos(a)*radius + math.random(-j*100,j*100)/100, math.sin(a)*radius + math.random(-j*100,j*100)/100, height + math.random(-j*100,j*100)/100
			elseif pattern == "Spiral" then
				local dr = radius + math.sin(a*0.5) * (Stranded.target_strafe.spiral_scale or 1) * radius
				x,z,y = math.cos(a)*dr, math.sin(a)*dr, math.sin(a) * (height*0.5)
			elseif pattern == "Randomized" then
				x,z,y = math.random(-radius*100,radius*100)/100, math.random(-radius*100,radius*100)/100, math.random(-math.abs(height)*100,math.abs(height)*100)/100
			elseif pattern == "Wave" then
				local t = now * speed * direction_mul
				x,z,y = math.cos(t)*radius, math.sin(t*2)*(radius*0.75), math.sin(t*3)*math.max(math.abs(height),1)
			elseif pattern == "Pulse" then
				local t = now * speed * direction_mul
				local p = (math.sin(t)+1)*0.5
				local dr = math.max(radius*p,1)
				x,z,y = math.cos(t)*dr, math.sin(t)*dr, height
			elseif pattern == "Dash" then
				local di = math.max(Stranded.target_strafe.dash_interval or 0.2, 0.05)
				if now >= (cache.next_dash or 0) then
					cache.next_dash = now + di
					cache.dash_dir = (math.random(0,1)==1) and 1 or -1
				end
				x,z,y = (Stranded.target_strafe.dash_distance or 5)*(cache.dash_dir or 1), math.sin(now*speed)*(radius*0.4), height
			elseif pattern == "Sway" then
				local t = now * speed * direction_mul
				x,z,y = math.cos(t)*radius, math.sin(t)*radius, height + math.sin(t*2)*(Stranded.target_strafe.sway_amount or 0)
			elseif pattern == "ZigZag" then
				local t = now * speed * direction_mul
				x,z,y = math.sin(t) * (Stranded.target_strafe.zigzag_width or 8), math.cos(t*0.6) * radius, height
			elseif pattern == "Flower" then
				local t = now * speed * direction_mul
				local petals = math.max(Stranded.target_strafe.flower_petals or 4, 2)
				local fr = radius * (0.5 + 0.5 * math.cos(t * petals))
				x,z,y = math.cos(t)*fr, math.sin(t)*fr, height
			else
				x,z,y = math.cos(a)*radius, math.sin(a)*radius, height
			end

			local vj = Stranded.target_strafe.vertical_jitter or 0
			local rj = Stranded.target_strafe.radius_jitter or 0
			if rj > 0 then x = x + math.random(-rj*100,rj*100)/100 z = z + math.random(-rj*100,rj*100)/100 end
			if vj > 0 then y = y + math.random(-vj*100,vj*100)/100 end

			local target_pos = target_part.Position
			local orbit_cf = CFrame.new(target_pos + Vector3.new(x,y,z), target_pos)
			if Stranded.target_strafe.server_only then
				local old_cf, old_vel = hrp.CFrame, hrp.AssemblyLinearVelocity
				hrp.CFrame = orbit_cf
				...
				rs:BindToRenderStep("TargetStrafeRestore", Enum.RenderPriority.Camera.Value - 2, function()
					if hrp and hrp.Parent then
						hrp.CFrame = old_cf
						hrp.AssemblyLinearVelocity = old_vel
					end
		...
		apply_desync = function()
			...
			local hrp = character:FindFirstChild("HumanoidRootPart")
			if not hrp then return end
			self.desync_cache = self.desync_cache or {last_velocity = Vector3.new(0,0,0), last_apply = 0}
			local now = tick()
			local interval = math.max(Stranded.desync.desync_interval or 0.05, 0)
			if interval > 0 and (now - (self.desync_cache.last_apply or 0)) < interval then return end
			self.desync_cache.last_apply = now
			self.desync_cache.last_cframe = hrp.CFrame
			self.desync_cache.last_velocity = hrp.AssemblyLinearVelocity

			local mode = Stranded.desync.mode or "Random"
			if self.desync_cache.last_mode ~= mode then
				self.desync_cache.last_mode = mode
				self.desync_cache.next_teleport = 0
				self.desync_cache.teleport_offset = nil
			end
			local off, yaw = Vector3.new(0,0,0), 0
			if mode == "Void" then
				local j = Stranded.desync.horizontal_jitter or 0
				off = Vector3.new(math.random(-j*100,j*100)/100, -math.abs(Stranded.desync.void_depth or 250), math.random(-j*100,j*100)/100)
			elseif mode == "Sky" then
				local j = Stranded.desync.horizontal_jitter or 0
				off = Vector3.new(math.random(-j*100,j*100)/100, math.abs(Stranded.desync.sky_height or 250), math.random(-j*100,j*100)/100)
			elseif mode == "Jitter" then
				local j = math.max(Stranded.desync.jitter_strength or 15, 1)
				off = Vector3.new(math.random(-j*100,j*100)/100, math.random(-j*100,j*100)/100, math.random(-j*100,j*100)/100)
			elseif mode == "Circle" or mode == "Helix" then
				local r = math.max(Stranded.desync.circle_radius or 12, 1)
				local spd = math.max((mode=="Helix" and Stranded.desync.helix_speed or Stranded.desync.circle_speed) or 8, 0.1)
				local a = now * spd
				off = Vector3.new(math.cos(a)*r, mode=="Helix" and math.sin(a*1.5)*(Stranded.desync.helix_height or 18) or 0, math.sin(a)*r)
				yaw = a
			elseif mode == "Spin" then
				off = Vector3.new(math.random(Stranded.desync.min_x*100,Stranded.desync.max_x*100)/100, math.random(Stranded.desync.min_y*100,Stranded.desync.max_y*100)/100, math.random(Stranded.desync.min_z*100,Stranded.desync.max_z*100)/100)
				yaw = math.rad((now * (Stranded.desync.yaw_spin_speed or 180)) % 360)
			elseif mode == "Pulse" then
				local p = (math.sin(now * math.max(Stranded.desync.circle_speed or 8, 0.1)) + 1) * 0.5
				local ps = math.max(Stranded.desync.pulse_strength or 12, 1)
				off = Vector3.new(math.random(Stranded.desync.min_x*100,Stranded.desync.max_x*100)/100 * ps * p, 0, math.random(Stranded.desync.min_z*100,Stranded.desync.max_z*100)/100 * ps * p)
			elseif mode == "Teleport" then
				local ti = math.max(Stranded.desync.teleport_interval or 0.3, 0.05)
				if now >= (self.desync_cache.next_teleport or 0) then
					local a = math.random() * math.pi * 2
					local r = math.max(Stranded.desync.teleport_radius or 45, 1)
					self.desync_cache.teleport_offset = Vector3.new(math.cos(a)*r, math.random(Stranded.desync.min_y,Stranded.desync.max_y), math.sin(a)*r)
					self.desync_cache.next_teleport = now + ti
				end
				off = self.desync_cache.teleport_offset or Vector3.new(0,0,0)
			elseif mode == "Line" then
				local line_speed = math.max(Stranded.desync.line_speed or 6, 0.1)
				local line_dist = math.max(Stranded.desync.line_distance or 20, 1)
				off = Vector3.new(math.sin(now * line_speed) * line_dist, 0, 0)
			elseif mode == "Burst" then
				local burst = math.max(Stranded.desync.burst_strength or 30, 1)
				off = Vector3.new(math.random(-burst*100, burst*100)/100, math.random(-burst*100, burst*100)/100, math.random(-burst*100, burst*100)/100)
			else
				off = Vector3.new(math.random(Stranded.desync.min_x*100,Stranded.desync.max_x*100)/100, math.random(Stranded.desync.min_y*100,Stranded.desync.max_y*100)/100, math.random(Stranded.desync.min_z*100,Stranded.desync.max_z*100)/100)
			end

			local fake_pos = self.desync_cache.last_cframe.Position + off
			local rot = math.rad(math.random(Stranded.desync.min_rotation, Stranded.desync.max_rotation))
			local _,cy = self.desync_cache.last_cframe:ToEulerAnglesYXZ()
			hrp.CFrame = CFrame.new(fake_pos) * CFrame.Angles(rot,0,0) * CFrame.Angles(0, cy + yaw, 0)

			local mag = Stranded.desync.max_velocity * math.max(Stranded.desync.velocity_scale or 1, 0)
			local dir = Vector3.new(math.random(-100,100)/100, math.random(-100,100)/100, math.random(-100,100)/100)
			if mode == "Circle" or mode == "Helix" then
				local a = now * math.max(Stranded.desync.circle_speed or 8, 0.1)
			...
			dir = dir.Magnitude < 0.001 and Vector3.new(0,1,0) or dir.Unit
			hrp.AssemblyLinearVelocity = dir * mag
...
local success_ui, err = pcall(function()
	...
	local target_strafe_target_dropdown
	local function refresh_target_strafe_targets()
		local items = {"Auto"}
		for _, player in ipairs(Players:GetPlayers()) do
			if player ~= Players.LocalPlayer then
				table.insert(items, (player.Name or "Unknown") .. " (@" .. (player.DisplayName or player.Name or "Unknown") .. ")")
			end
		end
		if target_strafe_target_dropdown and target_strafe_target_dropdown.Refresh then
			pcall(function() target_strafe_target_dropdown:Refresh(items) end)
		end
		if type(Stranded.target_strafe.selected_target) ~= "string" or not table.find(items, Stranded.target_strafe.selected_target) then
			Stranded.target_strafe.selected_target = "Auto"
		end
	end

	target_strafe_target_dropdown = TargetStrafeSection:Dropdown({Name = "Target", Flag = "Target Strafe Selected Target", Default = Stranded.target_strafe.selected_target, Items = {"Auto"}, Callback = function(Value)
		if Value then Stranded.target_strafe.selected_target = Value end
	end})
	TargetStrafeSection:Button({Name = "Refresh Target List", Callback = refresh_target_strafe_targets})
	Library:Connect(Players.PlayerAdded, refresh_target_strafe_targets, "Target Strafe Target Added")
	Library:Connect(Players.PlayerRemoving, refresh_target_strafe_targets, "Target Strafe Target Removed")
	...
	TargetStrafeSection:Dropdown({Name = "Pattern", Flag = "Target Strafe Pattern", Default = Stranded.target_strafe.pattern, Items = {"Normal", "Trigonometric", "Elliptical", "Figure-8", "Jitter", "Spiral", "Randomized", "Wave", "Pulse", "Dash", "Sway", "ZigZag", "Flower"}, Callback = function(Value)
		if Value then
	...
	TargetStrafeSection:Dropdown({Name = "Direction", Flag = "Target Strafe Direction", Default = Stranded.target_strafe.direction, Items = {"Clockwise", "Counter Clockwise"}, Callback = function(Value)
		if Value then Stranded.target_strafe.direction = Value end
	end})
	...
	TargetStrafeSection:Slider({Name = "Vertical Jitter", Min = 0, Max = 20, Default = Stranded.target_strafe.vertical_jitter, Decimals = 0, Compact = true, Flag = "Target Strafe Vertical Jitter", Callback = function(Value) Stranded.target_strafe.vertical_jitter = Value end})
	TargetStrafeSection:Slider({Name = "Radius Jitter", Min = 0, Max = 20, Default = Stranded.target_strafe.radius_jitter, Decimals = 0, Compact = true, Flag = "Target Strafe Radius Jitter", Callback = function(Value) Stranded.target_strafe.radius_jitter = Value end})
	TargetStrafeSection:Slider({Name = "Jitter Amount", Min = 0, Max = 25, Default = Stranded.target_strafe.jitter_amount, Decimals = 0, Compact = true, Flag = "Target Strafe Jitter Amount", Callback = function(Value) Stranded.target_strafe.jitter_amount = Value end})
	TargetStrafeSection:Slider({Name = "Randomize Interval", Min = 0.05, Max = 1, Default = Stranded.target_strafe.randomize_interval, Decimals = 2, Compact = true, Flag = "Target Strafe Randomize Interval", Callback = function(Value) Stranded.target_strafe.randomize_interval = Value end})
	TargetStrafeSection:Slider({Name = "Spiral Scale", Min = 0.1, Max = 3, Default = Stranded.target_strafe.spiral_scale, Decimals = 2, Compact = true, Flag = "Target Strafe Spiral Scale", Callback = function(Value) Stranded.target_strafe.spiral_scale = Value end})
	TargetStrafeSection:Slider({Name = "Sway Amount", Min = 0, Max = 20, Default = Stranded.target_strafe.sway_amount, Decimals = 0, Compact = true, Flag = "Target Strafe Sway Amount", Callback = function(Value) Stranded.target_strafe.sway_amount = Value end})
	TargetStrafeSection:Slider({Name = "ZigZag Width", Min = 1, Max = 25, Default = Stranded.target_strafe.zigzag_width, Decimals = 0, Compact = true, Flag = "Target Strafe ZigZag Width", Callback = function(Value) Stranded.target_strafe.zigzag_width = Value end})
	TargetStrafeSection:Slider({Name = "Flower Petals", Min = 2, Max = 10, Default = Stranded.target_strafe.flower_petals, Decimals = 0, Compact = true, Flag = "Target Strafe Flower Petals", Callback = function(Value) Stranded.target_strafe.flower_petals = Value end})
	TargetStrafeSection:Slider({Name = "Dash Interval", Min = 0.05, Max = 1, Default = Stranded.target_strafe.dash_interval, Decimals = 2, Compact = true, Flag = "Target Strafe Dash Interval", Callback = function(Value) Stranded.target_strafe.dash_interval = Value end})
	TargetStrafeSection:Slider({Name = "Dash Distance", Min = 1, Max = 30, Default = Stranded.target_strafe.dash_distance, Decimals = 0, Compact = true, Flag = "Target Strafe Dash Distance", Callback = function(Value) Stranded.target_strafe.dash_distance = Value end})
	TargetStrafeSection:Toggle({Name = "Mobile Keybind Bypass", Flag = "Target Strafe Mobile Support", Default = Stranded.target_strafe.mobile_support, Callback = function(Value) Stranded.target_strafe.mobile_support = Value end})
	TargetStrafeSection:Toggle({Name = "Server Position Strafe", Flag = "Target Strafe Server Only", Default = Stranded.target_strafe.server_only, Callback = function(Value) Stranded.target_strafe.server_only = Value end})
	...
	local update_desync_visibility
	local desync_mode_dropdown = DesyncSection:Dropdown({Name = "Mode", Flag = "Desync Mode", Default = Stranded.desync.mode, Items = {"Random", "Void", "Sky", "Jitter", "Circle", "Spin", "Helix", "Pulse", "Teleport", "Line", "Burst"}, Callback = function(Value)
		if Value then
			Stranded.desync.mode = Value
			if esp and esp.desync_cache then
				esp.desync_cache.last_mode = nil
			end
			if update_desync_visibility then
				update_desync_visibility(Value)
			end
		end
	end})
	
	local desync_min_x = DesyncSection:Slider({Name = "Min X", Min = -100, Max = 100, Default = Stranded.desync.min_x, Decimals = 0, Compact = true, Flag = "Desync Min X", Callback = function(Value)
		Stranded.desync.min_x = Value
	end})
	
	local desync_max_x = DesyncSection:Slider({Name = "Max X", Min = -100, Max = 100, Default = Stranded.desync.max_x, Decimals = 0, Compact = true, Flag = "Desync Max X", Callback = function(Value)
		Stranded.desync.max_x = Value
	end})
	
	local desync_min_y = DesyncSection:Slider({Name = "Min Y", Min = -100, Max = 100, Default = Stranded.desync.min_y, Decimals = 0, Compact = true, Flag = "Desync Min Y", Callback = function(Value)
		Stranded.desync.min_y = Value
	end})
	
	local desync_max_y = DesyncSection:Slider({Name = "Max Y", Min = -100, Max = 100, Default = Stranded.desync.max_y, Decimals = 0, Compact = true, Flag = "Desync Max Y", Callback = function(Value)
		Stranded.desync.max_y = Value
	end})
	
	local desync_min_z = DesyncSection:Slider({Name = "Min Z", Min = -100, Max = 100, Default = Stranded.desync.min_z, Decimals = 0, Compact = true, Flag = "Desync Min Z", Callback = function(Value)
		Stranded.desync.min_z = Value
	end})
	
	local desync_max_z = DesyncSection:Slider({Name = "Max Z", Min = -100, Max = 100, Default = Stranded.desync.max_z, Decimals = 0, Compact = true, Flag = "Desync Max Z", Callback = function(Value)
		Stranded.desync.max_z = Value
	end})

	local desync_void_depth = DesyncSection:Slider({Name = "Void Depth", Min = 25, Max = 1000, Default = Stranded.desync.void_depth, Decimals = 0, Compact = true, Flag = "Desync Void Depth", Callback = function(Value) Stranded.desync.void_depth = Value end})
	local desync_sky_height = DesyncSection:Slider({Name = "Sky Height", Min = 25, Max = 1000, Default = Stranded.desync.sky_height, Decimals = 0, Compact = true, Flag = "Desync Sky Height", Callback = function(Value) Stranded.desync.sky_height = Value end})
	local desync_horizontal_jitter = DesyncSection:Slider({Name = "Horizontal Jitter", Min = 0, Max = 100, Default = Stranded.desync.horizontal_jitter, Decimals = 0, Compact = true, Flag = "Desync Horizontal Jitter", Callback = function(Value) Stranded.desync.horizontal_jitter = Value end})
	local desync_jitter_strength = DesyncSection:Slider({Name = "Jitter Strength", Min = 1, Max = 100, Default = Stranded.desync.jitter_strength, Decimals = 0, Compact = true, Flag = "Desync Jitter Strength", Callback = function(Value) Stranded.desync.jitter_strength = Value end})
	local desync_circle_radius = DesyncSection:Slider({Name = "Circle Radius", Min = 1, Max = 100, Default = Stranded.desync.circle_radius, Decimals = 0, Compact = true, Flag = "Desync Circle Radius", Callback = function(Value) Stranded.desync.circle_radius = Value end})
	local desync_circle_speed = DesyncSection:Slider({Name = "Circle Speed", Min = 1, Max = 50, Default = Stranded.desync.circle_speed, Decimals = 0, Compact = true, Flag = "Desync Circle Speed", Callback = function(Value) Stranded.desync.circle_speed = Value end})
	local desync_helix_height = DesyncSection:Slider({Name = "Helix Height", Min = 1, Max = 100, Default = Stranded.desync.helix_height, Decimals = 0, Compact = true, Flag = "Desync Helix Height", Callback = function(Value) Stranded.desync.helix_height = Value end})
	local desync_helix_speed = DesyncSection:Slider({Name = "Helix Speed", Min = 1, Max = 50, Default = Stranded.desync.helix_speed, Decimals = 0, Compact = true, Flag = "Desync Helix Speed", Callback = function(Value) Stranded.desync.helix_speed = Value end})
	local desync_pulse_strength = DesyncSection:Slider({Name = "Pulse Strength", Min = 1, Max = 100, Default = Stranded.desync.pulse_strength, Decimals = 0, Compact = true, Flag = "Desync Pulse Strength", Callback = function(Value) Stranded.desync.pulse_strength = Value end})
	local desync_teleport_radius = DesyncSection:Slider({Name = "Teleport Radius", Min = 1, Max = 200, Default = Stranded.desync.teleport_radius, Decimals = 0, Compact = true, Flag = "Desync Teleport Radius", Callback = function(Value) Stranded.desync.teleport_radius = Value end})
	local desync_teleport_interval = DesyncSection:Slider({Name = "Teleport Interval", Min = 0.05, Max = 1, Default = Stranded.desync.teleport_interval, Decimals = 2, Compact = true, Flag = "Desync Teleport Interval", Callback = function(Value) Stranded.desync.teleport_interval = Value end})
	local desync_spin_speed = DesyncSection:Slider({Name = "Spin Speed", Min = 1, Max = 1000, Default = Stranded.desync.yaw_spin_speed, Decimals = 0, Compact = true, Flag = "Desync Spin Speed", Callback = function(Value) Stranded.desync.yaw_spin_speed = Value end})
	local desync_update_interval = DesyncSection:Slider({Name = "Update Interval", Min = 0, Max = 1, Default = Stranded.desync.desync_interval, Decimals = 2, Compact = true, Flag = "Desync Update Interval", Callback = function(Value) Stranded.desync.desync_interval = Value end})
	local desync_velocity_scale = DesyncSection:Slider({Name = "Velocity Scale", Min = 0, Max = 5, Default = Stranded.desync.velocity_scale, Decimals = 2, Compact = true, Flag = "Desync Velocity Scale", Callback = function(Value) Stranded.desync.velocity_scale = Value end})

	local desync_line_distance = DesyncSection:Slider({Name = "Line Distance", Min = 1, Max = 100, Default = Stranded.desync.line_distance, Decimals = 0, Compact = true, Flag = "Desync Line Distance", Callback = function(Value) Stranded.desync.line_distance = Value end})
	local desync_line_speed = DesyncSection:Slider({Name = "Line Speed", Min = 1, Max = 30, Default = Stranded.desync.line_speed, Decimals = 0, Compact = true, Flag = "Desync Line Speed", Callback = function(Value) Stranded.desync.line_speed = Value end})
	local desync_burst_strength = DesyncSection:Slider({Name = "Burst Strength", Min = 1, Max = 100, Default = Stranded.desync.burst_strength, Decimals = 0, Compact = true, Flag = "Desync Burst Strength", Callback = function(Value) Stranded.desync.burst_strength = Value end})


	update_desync_visibility = function(mode)
		mode = mode or Stranded.desync.mode or "Random"
		local function show(ctrl, visible)
			if ctrl and ctrl.SetVisibility then ctrl:SetVisibility(visible) end
		end
		local is_random = mode == "Random" or mode == "Spin" or mode == "Burst"
		show(desync_min_x, is_random)
		show(desync_max_x, is_random)
		show(desync_min_y, is_random)
		show(desync_max_y, is_random)
		show(desync_min_z, is_random)
		show(desync_max_z, is_random)
		show(desync_horizontal_jitter, mode == "Void" or mode == "Sky")
		show(desync_void_depth, mode == "Void")
		show(desync_sky_height, mode == "Sky")
		show(desync_jitter_strength, mode == "Jitter")
		show(desync_circle_radius, mode == "Circle" or mode == "Helix")
		show(desync_circle_speed, mode == "Circle" or mode == "Helix" or mode == "Pulse")
		show(desync_helix_height, mode == "Helix")
		show(desync_helix_speed, mode == "Helix")
		show(desync_pulse_strength, mode == "Pulse")
		show(desync_teleport_radius, mode == "Teleport")
		show(desync_teleport_interval, mode == "Teleport")
		show(desync_spin_speed, mode == "Spin")
		show(desync_line_distance, mode == "Line")
		show(desync_line_speed, mode == "Line")
		show(desync_burst_strength, mode == "Burst")
		show(desync_update_interval, true)
		show(desync_velocity_scale, true)
	end
	update_desync_visibility(Stranded.desync.mode)
